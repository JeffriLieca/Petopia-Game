...
class WandAgentNode : SKSpriteNode, GKAgentDelegate{
    var agent :GKAgent2D?
    let petCategory: UInt32 = 0x1 << 0
    let wandCategory: UInt32 = 0x1 << 1
    let wallCategory: UInt32 = 0x1 << 2
    
    var visualAgent : SKSpriteNode?
    
    private var wandFrame: SKTexture = SKTexture(imageNamed: "wand")
 
    init( scene: SKScene, position: CGPoint, size: CGSize) {
        
        
//        let wand = SKSpriteNode(imageNamed: "wand")
//        wand.size = CGSize (width: 100, height: 100)
////        wand.position = CGPoint(x: game.frame.midX, y: game.frame.midY-400)
//        wand.zPosition = 10000000
//        wand.physicsBody = SKPhysicsBody(circleOfRadius: 50)
//        wand.physicsBody = SKPhysicsBody(texture: SKTexture(imageNamed: "wand"), size: wand.size)
//        wand.name = "wand"
//        wand.physicsBody?.restitution = 0.7
//        wand.physicsBody?.categoryBitMask = wandCategory
//        wand.physicsBody?.collisionBitMask = petCategory
//        wand.physicsBody?.contactTestBitMask = petCategory
//        scene.addChild(wand)
        
        // Call the designated initializer of SKSpriteNode with a color and size
               let color = SKColor.clear
               let size = size
        super.init(texture: self.wandFrame, color: color, size: size)
        
        self.agent = GKAgent2D()
        self.agent!.position = vector_float2(x: Float(position.x - self.size.width/2), y: Float(position.y))
...
//        self.physicsBody = SKPhysicsBody(texture: self.texture!, size: self.size)
        ...
        self.physicsBody?.categoryBitMask = wandCategory
...
//        self.wandFrame = wandFrame
    ...
    func agentWillUpdate(_ agent: GKAgent) {
        ...
        if let agent2d = agent as? GKAgent2D {
            agent2d.position = SIMD2(Float(self.position.x - self.size.width/2), Float(self.position.y))
        }
